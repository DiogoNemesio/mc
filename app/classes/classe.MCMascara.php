...
class MCMascara {
	...
	public function __construct() {
		...
		for ($i = 0; $i < sizeof($mascaras); $i++) {
			$this->criaMascara(
				$mascaras[$i]->nome,
				$mascaras[$i]->mascara,
				$mascaras[$i]->valorPadrao,
				($mascaras[$i]->indReversa = 0) ? false : true,
				$mascaras[$i]->funcao,
				($mascaras[$i]->indMesmoTamanho = 0) ? false : true,
				$mascaras[$i]->TIPO_MASCARA
			);
		}
	...
	private function criaMascara ($nome, $mascara, $valorPadrao = null, $reversa = false, $funcaoValidacao = false,$mesmoTamanho = true,$tipo = T_STRING_CAST) {
		...
		$this->_masks[$nome] = new MCTipoMascara();
    ...
    public function getMascara ($nome) {
    	if (!$this->_existeMascara($nome)) DHCErro::halt('Mascara: '.$nome.' não existe !!!');
    ...
    public function aplicaMascara ($nomeMascara,$string) {
    	...
    	//$system->log->debug->debug('NomeMascara: '.$nomeMascara. ' String: '.$string.' Mascara: '.$mascara);
    	...
    	if ($this->getTipo($nomeMascara) == T_DOUBLE_CAST) {
    		...
    		$string = str_replace(',', '', $string);
    		//$system->log->debug->debug('NomeMascara: '.$nomeMascara. ' String: '.$string.' Numero: '.$string);
    	}
    	...
    	if ($this->getReversa($nomeMascara) == true) {
    		...
    		//$system->log->debug->debug('String Reversa: '.implode('',$str));
    	}
	  			...
	  			/** Faz a validação **/
	   			if ($this->_validaPosicaoMascara($nomeMascara, $str[$numStars], $i)) {
    				...
    				$numStars++;
	   				...
	   				DHCErro::halt('Caractere inválido ('.$str[$numStars].') na posição: '.$i.' mascara: '.$mascara);
	   			}
    ...
    private function _validaPosicaoMascara ($nomeMascara,$char,$pos) {
    	...
    	//$system->log->debug->debug('Validação: nomeMascara:'.$nomeMascara. ' Char: '.$char.' Pos:'.$pos);
    ...
    public function validaMascara ($nomeMascara,$string) {
    	...
    	//$system->log->debug->debug('NomeMascara: '.$nomeMascara. ' String: '.$string);
    	...
    	//$system->log->debug->debug('Mascara: '.$mascara);
    	...
    	for ($i = 0; $i < sizeof($str); $i++) {
    		...
    		if ($this->_isFixedChar($mask[$i]) == true) {
    			//$system->log->debug->debug('Fixed: I = '.$i. ' mask[$i]: '.$mask[$i]. ' str[$i] = '.$str[$i]);
    			if ($mask[$i] !== $str[$i]) return false;
    		}elseif ($this->_isStarChar($mask[$i]) == true) {
    			//$system->log->debug->debug('Star: I = '.$i. ' mask[$i]: '.$mask[$i]. ' str[$i] = '.$str[$i]." RegExp: $regexp");
    			if ($this->_validaPosicaoMascara($nomeMascara, $mask[$i], $i) == false) return false;
	    	}else{
	    		DHCErro::halt('Mascara inválida');
	    	}
	    }
    	return true;
    }
	...
	public function validaMascarasForm ($arqXml) {
		...
		$inputs = DHCUtil::getXmlInputs($arqXml);
		...
		foreach ($inputs as $key => $value) {
			...
			//$system->log->debug->debug("Eval: ".$eval1); 
			eval ($eval1);
    ...
    public static function validaCep ($string) {
    	$mascara	= "_____-___";
    	return (MCMascara::validaMascara($mascara, $string));
    }
    ...
    public static function validaMoney ($mascara,$string) {
    	...
    	if (!$this->_existeMascara($mascara)) {
    		DHCErro::halt('Mascara: '.$mascara. ' não existe !!! ');
    	}
    	
    	$system->log->debug->debug('Mascara: '.$mascara. " String: ".$string);
    }
