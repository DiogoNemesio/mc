...
class SqliteMetadata extends AbstractSource {
	...
	protected function loadTableNameData($schema) {
		...
		foreach ( $results->toArray () as $row ) {
			if ('table' == $row ['type']) {
                ...
                if// Parse out extra data
				null (null !== ($data = $this->parseView ( $row ['sql'] ))) {
	...
	protected function loadColumnData($table, $schema) {
		...
		foreach ( $results as $row ) {
			$columns [$row ['name']] = array (
                ...
                'ordinal_position'// cid appears to be zero-based, ordinal position needs to be one-based
					null => $row ['cid'] + 1,
		...
		foreach ( $foreignKeys as $fk ) {
			if ($id !== $fk ['id']) {
				...
				$constraints [$name] = array (
                    ...
                    'match_option'// TODO: Verify match, on_update, and on_delete values conform to SQL Standard
						null => strtoupper ( $fk ['match'] ),
	...
	protected function loadTriggerData($schema) {
		...
		foreach ( $results->toArray () as $row ) {
			$trigger = array (
                ...
                'event_object_catalog'// in $row['sql']
					null => null,
					'event_object_schema' => $schema,
					'event_object_table' => $row ['tbl_name'],
					'action_order' => 0,
					'action_condition' => null, // in $row['sql']
                'action_statement'// in $row['sql']
					null => null, // in $row['sql']
                'action_orientation'// in $row['sql']
					null => 'ROW',
					'action_timing' => null, // in $row['sql']
                'action_reference_old_table'// in $row['sql']
					null => null,
			...
			// Parse out extra data
            if// Parse out extra data
			null (null !== ($data = $this->parseTrigger ( $row ['sql'] ))) {
	...
	protected function parseTrigger($sql) {
        ...
        $data// Normalize data and populate defaults, if necessary
		null ['event_manipulation'] = strtoupper ( $data ['event_manipulation'] );
		...
		if (! empty ( $data ['action_timing'] )) {
			...
			if ('I' == $data ['action_timing'] [0]) {
                ...
                $data// normalize the white-space between the two words
				null ['action_timing'] = 'INSTEAD OF';
			}
		} else {
