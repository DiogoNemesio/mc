...
class AnnotationBuilder implements EventManagerAwareInterface, FormFactoryAwareInterface {
	...
	protected function configureElement($annotations, $reflection, $formSpec, $filterSpec) {
        ...
        if// If the element is marked as exclude, return early
		null ($this->checkForExclude ( $annotations )) {
			return;
		}
        ...
        if// In all other cases, though, the name is fine.
		null ($name === 'type') {
        ...
        if// Compose as a fieldset or an element, based on specification type
		null (static::isSubclassOf ( $type, 'Zend\Form\FieldsetInterface' )) {
