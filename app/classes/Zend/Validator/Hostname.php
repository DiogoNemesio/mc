...
class Hostname extends AbstractValidator {
	 ...
	 *        	Set whether the TLD element of a hostname is validated (default true)
				...
				if (preg_match ( '/([^.]{2,10})$/i', end ( $domainParts ), $matches ) || (array_key_exists ( end ( $domainParts ), $this->validIdns ))) {
					...
					foreach ( $domainParts as $domainPart ) {
						...
						foreach ( $regexChars as $regexKey => $regexChar ) {
							...
							if ($status > 0) {
								$length = 63;
								if (array_key_exists ( strtoupper ( $this->tld ), $this->idnLength ) && (array_key_exists ( $regexKey, $this->idnLength [strtoupper ( $this->tld )] ))) {
