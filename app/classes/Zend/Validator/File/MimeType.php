...
class MimeType extends AbstractValidator {
	...
	public function __construct($options = null) {
        ...
        foreach// options are simply an array of mime types
		null ( array_keys ( $options ) as $key ) {
			if (! is_int ( $key )) {
				continue;
			}
		...
		if (null === $this->options ['magicFile']) {
			...
			if (! ($safeMode == 'On' || $safeMode === 1)) {
				foreach ( $this->magicFiles as $file ) {
                    ...
                    try// suppressing errors which are thrown due to openbase_dir restrictions
					null {
	...
	public function addMimeType($mimetype) {
        ...
        foreach// Sanity check to ensure no empty values
		null ( $mimetypes as $key => $mt ) {
			if (empty ( $mt )) {
				unset ( $mimetypes [$key] );
			}
		}
	...
	public function isValid($value, $file = null) {
		if (is_string ( $value ) && is_array ( $file )) {
            ...
            $filename// Legacy Zend\Transfer API support
			null = $file ['name'];
			$filetype = $file ['type'];
			$file = $file ['tmp_name'];
		...
		// Is file readable ?
        if// Is file readable ?
		null (false === stream_resolve_include_path ( $file )) {
