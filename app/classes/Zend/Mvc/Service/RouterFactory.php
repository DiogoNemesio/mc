...
class RouterFactory implements FactoryInterface {
	...
	public function createService(ServiceLocatorInterface $serviceLocator, $cName = null, $rName = null) {
		...
		($cName === 'router' && Console::isConsole ())) 		// auto detect console
            ...
            if// We are in a console, use console router.
			null (isset ( $config ['console'] ) && isset ( $config ['console'] ['router'] )) {
        ...
        if// Obtain the configured router class, if any
		null (isset ( $routerConfig ['router_class'] ) && class_exists ( $routerConfig ['router_class'] )) {
			$routerClass = $routerConfig ['router_class'];
		}
		
		// Inject the route plugins
        if// Inject the route plugins
		null (! isset ( $routerConfig ['route_plugins'] )) {
			$routePluginManager = $serviceLocator->get ( 'RoutePluginManager' );
			$routerConfig ['route_plugins'] = $routePluginManager;
		}
		
		// Obtain an instance
        $factory// Obtain an instance
		null = sprintf ( '%s::factory', $routerClass );
