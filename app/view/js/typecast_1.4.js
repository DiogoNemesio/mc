...
var Typecast = {
	...
	Behaviours : {
		Mask : {
			...
			InitField : function(field){
				var fieldData = [];
				var TCMask		= Typecast.Utils.GetFieldMask(field);
				fieldData = eval("Typecast.Config.Data.Mask.Masks." + TCMask);
				if (field.value != null) {
					fieldData += Typecast.Config.Settings.Mask.FieldDataSeparator + field.value; 
				} 
			...
			KeyHandler : function(e){
				...
				if(e.keyCode==8 && e.type=="keydown" && this.AllowInsert){
					...
					if (this.isReverse) {
						//mask.DataManager.MoveLeft(this,postBackspaceCursorPosition);
						mask.DataManager.MoveLeft(this);
						mask.CursorManager.Move(this, 1);
					}else{
				...
				//Esc
					...
					mask.DataManager.clearField(this);
					mask.Render(this);
				}
				...
				//End
					...
					if (this.isReverse) {
						var startIdx = this.MaskIndex[this.MaskIndex.length-1]+1;
					}else{
				...
				//Home
					...
					if (this.isReverse) {
						var startIdx = Typecast.Behaviours.Mask.MaskManager.FindNearestMaskCharacter(this, parseInt(this.DataIndex[0]), -1);
					}else{
						var startIdx = this.MaskIndex[0];
					}
					alert(startIdx);
					Typecast.Behaviours.Mask.CursorManager.SetPosition(this, startIdx);
				}
				...
				//Numeric Characters
					...
					//alert('DataIndex: \"'+this.DataIndex+'\"');
				}
			...
			MouseUp : function(e) {
				...
				if (this.isReverse) {
					var startIdx = Typecast.Behaviours.Mask.MaskManager.FindNearestMaskCharacter(this, cursorPosition, -1)+1;
				}else{
			...
			ParseFieldData : function(field, fieldData){
				...
				field.Data 				= [];
				field.MaskName			= Typecast.Utils.GetFieldMask(field);
				field.isReverse			= (field.MaskName.substr(0,5) == 'money') ? true : false;
				field.DataIndex 		= [];
			...
			MaskManager : {
				...
				CurrentMaskCharacter : function(field){
					if (field.isReverse) {
						var cursorPosition = parseInt(Typecast.Behaviours.Mask.CursorManager.GetPosition(field)[0])-1;
					}else{
				...
				FindNearestMaskCharacter : function(field, cursorPosition, dir){
					if (field.isReverse) {
						var nearestMaskCharacter = (field.DataIndex.length > 0) ? cursorPosition : field.MaskIndex[field.MaskIndex.length-1];
					}else{
			...
			CursorManager : {
				...
				GetPosition : function(field){
					if (field.isReverse) {
						var arr = [parseInt(field.MaskIndex[field.MaskIndex.length-1])+1,parseInt(field.MaskIndex[field.MaskIndex.length-1])+1];
					}else{
						var arr = [0,0];
					}
				...
				TabbedInSetPosition : function(field){
					...
					if(mask.MaskManager.CurrentMaskCharacter(field) == undefined){
						...
						if(field.DataIndex.length > 0 && field.DataIndex.length != field.MaskIndex.length){
							(field.isReverse) ? startIdx = mask.MaskManager.FindNearestMaskCharacter(field, mask.CursorManager.GetPosition(field)[0], -1)+1 : startIdx = parseInt(field.MaskIndex[field.DataIndex.length]);
						}else if(field.DataIndex.length == field.MaskIndex.length){
							(field.isReverse) ? startIdx = mask.MaskManager.FindNearestMaskCharacter(field, mask.CursorManager.GetPosition(field)[0], -1)+1 : startIdx = parseInt(field.DataIndex[field.DataIndex.length-1]) + 1;
							alert(startIdx);
						}else{
							(field.isReverse) ? startIdx = parseInt(field.MaskIndex[field.MaskIndex.length-1])+1 : startIdx = field.MaskIndex[0];
						}
						this.SetPosition(field, startIdx);
					}
				},
			...
			DataManager : {
				...
				InsertCharacter : function(field, char){
					...
					if (field.isReverse) {
						this.MoveRight(field,currentCharacterPosition);
						field.Data[parseInt(field.MaskIndex[currentCharacterPosition])] = char;
					}else{
				...
				MoveRight : function(field,currentCharacterPosition){
					for (i = 1; i < currentCharacterPosition + 1; i++) {
						field.Data[field.MaskIndex[i-1]] = field.Data[field.MaskIndex[i]];
					}
				},
				MoveLeft : function(field){
					var currentCharacterPosition = this.CurrentDataIndexPosition(field);
					for (i = currentCharacterPosition+1; i > 0; i--) {
				...
				CurrentDataIndexPosition : function(field){
					var cursorPosition = Typecast.Behaviours.Mask.CursorManager.GetPosition(field)[0];
					if (field.isReverse) {
						var currentDataIndexPosition = field.MaskIndex.length-1;
					}else{
				...
				clearField : function (field) {
					field.value 	= null;
					Typecast.Behaviours.Mask.InitField(field);
				}
			},
	...
	Utils : {
		...
		GetFieldMask : function(field){
			var mask		= field.className.substring(field.className.indexOf("[")+1, field.className.indexOf("]"));
			return mask;
		},
